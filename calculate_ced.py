        ...
        for img_name in predicted_points[method_name].keys():
            if img_name in gt_points:
                ...
                if args.normalization_type == "eyes":
                    ...
                    if (
                        len(left_eye_idx) == 1 and len(right_eye_idx) == 1
                    ) or (len(left_eye_idx) == 2 and len(right_eye_idx) == 2):
                        x_left_eye = np.mean(
                            [x_gt[int(idx)] for idx in left_eye_idx]
                        )
                        x_right_eye = np.mean(
                            [x_gt[int(idx)] for idx in right_eye_idx]
                        )
                        y_left_eye = np.mean(
                            [y_gt[int(idx)] for idx in left_eye_idx]
                        )
                        y_right_eye = np.mean(
                            [y_gt[int(idx)] for idx in right_eye_idx]
                        )
                    else:
                ...
                avg_norm_dist = np.sum(dist) / (
                    n_points * normalization_factor
                )
                ceds[method_name].append(avg_norm_dist)
            else:
                print(
                    "Skipping key {}, because its not in the gt points".format(
                        img_name
                    )
                )
...
def count_ced_auc(errors):
    ...
    for error in errors:
        auc = 0
        proportions = (
            np.arange(error.shape[0], dtype=np.float32) / error.shape[0]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="CED computation script", add_help=True
    )
    parser.add_argument("--gt_path", action="store", type=str, help="")
    parser.add_argument(
        "--predictions_path", action="append", type=str, help=""
    )
    ...
    parser.add_argument(
        "--max_points_to_read", action="store", type=int, help="", default=None
    )

    parser.add_argument(
        "--error_thr", action="store", type=float, help="", default=0.08
    )
